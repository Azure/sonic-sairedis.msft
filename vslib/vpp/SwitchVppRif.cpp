    ...
    _In_ const sai_attribute_t *attr_list)
{
    ...
    else if (objectTypeQuery(object_id) == SAI_OBJECT_TYPE_LAG)
    {
        ...
        hwifname = std::string(BONDETHERNET_PREFIX) + std::to_string(bond_info.id);
    }
    else
    {
        std::string tap_name;
        if (getTapNameFromPortId(object_id, tap_name) == false)
        {
            SWSS_LOG_ERROR("host interface for port id %s not found",
